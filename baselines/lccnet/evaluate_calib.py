...
# import MultisenseDataset.py
import importlib.util
spec = importlib.util.spec_from_file_location("module.name", "/home/ios/GitLab/rgbd-registration/rgb-lidar-reg/datasets/MultiSenseDatasetLCCNet.py")
multisense = importlib.util.module_from_spec(spec)
spec.loader.exec_module(multisense)
...
@ex.config
def config():
    dataset = 'kitti/raw'#'kitti/odom'
    #data_folder = '/home/ios/data2/kitti/odometry/dataset'
    data_folder = '/home/ios/data3/multisense_stereo'
    ...
    max_t = 0.05 #0.01 #1e-8 # #
    max_r = 1.87 # (euclidean of [0.40, 1.82, 0.09])
    occlusion_kernel = 5
...
def main(_config, seed):
    ...
    dataset_class = multisense.MultiSenseData
    ...
    if _config['test_sequence'] is None:
        ...
        # if MultiSenseData
        if dataset_class == multisense.MultiSenseData:
          dataset_val = dataset_class(_config['data_folder'], rectified=True, crop=[0, 0, -1, -1], full=True, max_r=_config['max_r'], max_t=_config['max_t'])
        else:
          # old code from here
          if isinstance(_config['test_sequence'], int):
              _config['test_sequence'] = f"{_config['test_sequence']:02d}"
              dataset_val = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'], split='test', use_reflectance=_config['use_reflectance'], val_sequence=_config['test_sequence'])
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            if _config['save_image']:
                ...
                img = sample['rgb'][0].numpy().astype(np.uint8)
        ...
        rgb_resize = F.interpolate(rgb_input, size=[256, 512])
        lidar_resize = F.interpolate(lidar_input, size=[256, 512])
        ...
        if _config['save_image']:
            ...
            cv2.imwrite(os.path.join(depth_path, sample['rgb_name'][0]), depth_img[0, :1199, :1919])
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                lidar_resize = F.interpolate(lidar, size=[256, 512])
